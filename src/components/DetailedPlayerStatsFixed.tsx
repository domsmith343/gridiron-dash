...
import { FantasyPlayer, PlayerGameStats, PlayerTrend } from '../types';
...
const DetailedPlayerStats: React.FC<DetailedPlayerStatsProps> = memo(({ player, className = '' }) => {
  ...
  const getRadarChartData = useCallback(() => {
    ...
    if (player.position === 'QB') {
      ...
      radarData.push({ category: 'Rec TDs', value: (stats.touchdowns || 0), maxValue: 2 });
    } else if (player.position === 'WR' || player.position === 'TE') {
      ...
      radarData.push({ category: 'Rec TDs', value: (stats.touchdowns || 0), maxValue: 3 });
      ...
      <Card key={trend.category} className="mb-4">
        <CardContent className="p-4">
          <div className="flex justify-between items-center mb-3">
            <h5 className="text-sm font-medium">{trend.category}</h5>
          ...
          <div className="h-16 flex items-end space-x-2">
            {trend.data.map((value: number, i: number) => (
              <div 
                ...
                role="img"
                aria-label={`${trend.category}: ${value} in game ${i + 1}`}
              >
                <div 
              ...
              <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                {gameLog.map((game: PlayerGameStats, index: number) => (
                  <tr 
                    ...
                    role="row"
                  >
                    <td className="px-3 py-2 text-xs" role="gridcell">{game.date}</td>
                    <td className="px-3 py-2 text-xs" role="gridcell">{game.opponent}</td>
                    {player.position === 'QB' && (
                      <>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.passingYards}</td>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.passingTDs}</td>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.interceptions}</td>
                      </>
                    )}
                    {(player.position === 'RB' || player.position === 'QB') && (
                      <>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.rushingYards}</td>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.rushingTDs}</td>
                      </>
                    )}
                    {(player.position === 'WR' || player.position === 'TE' || player.position === 'RB') && (
                      <>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.receptions}</td>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.receivingYards}</td>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.receivingTDs}</td>
                      </>
                    )}
                    {player.position === 'K' && (
                      <>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.fieldGoals}</td>
                        <td className="px-3 py-2 text-xs" role="gridcell">{game.extraPoints}</td>
                      </>
                    )}
                    <td className="px-3 py-2 text-xs font-medium" role="gridcell">{game.fantasyPoints}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  // Tab handlers with useCallback for performance
  const handleTabChange = useCallback((tab: 'overview' | 'gameLog' | 'trends') => {
    setActiveTab(tab);
  }, []);
  
  return (
    <div className={`detailed-player-stats ${className}`} {...generateAriaAttributes({
      role: 'region',
      ariaLabel: `Detailed statistics for ${player.name}`
    })}>
      {/* Player Header */}
      <Card className="mb-6">
        <CardContent className="p-6">
          <div className="flex items-center">
            <div className="flex-shrink-0 mr-4">
              <img
                src={player.avatarUrl || `https://ui-avatars.com/api/?name=${player.name.split(' ').map((n: string) => n[0]).join('')}&background=random`}
      ...
      <TabNavigation
        tabs={[
          ...
          { id: 'gameLog', label: 'Game Log', content: null },
          { id: 'trends', label: 'Trends', content: null }
        ]}
        activeTab={activeTab}
      ...
      <div role="tabpanel" aria-labelledby={`tab-${activeTab}`}>
        ...
        {activeTab === 'gameLog' && (
          <div>
            <h4 className="text-lg font-semibold mb-4">Recent Game Log</h4>
            {renderGameLogTable()}
          </div>
        )}
